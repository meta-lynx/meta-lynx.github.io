...
def load_models(model_name, model_weights=None, number_classes=1000):
    ...
    if model_name == 'AlexNet':
        ...
        raise ValueError('Unsupported model. Please choose one of AlexNet, ResNet18, VGG16, \
            SqueezeNet, DenseNet161, InceptionV3, GoogleNet, MobileNetV2, MobileNetV3L, MobileNetV3S')
...
def extract_embedding_coordinates(model, image_list, device, transform):
...
def main():
    ...
    # if model exists
    coords = extract_embedding_coordinates(feature_model, image_list, device, transform_name) # TSNE Coordinates
